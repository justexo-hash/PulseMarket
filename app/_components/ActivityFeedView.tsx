...
ActivityFeedView() {
        ...
        const activityDate =
          activity.type === "bet"
            ? activity.data.createdAt
              ? new Date(activity.data.createdAt)
              : null
            : activity.data.createdAt
            ? new Date(activity.data.createdAt)
            : null;
  ...
  if (activities.isLoading || markets.length === 0) {
    return (
      <div className="">
        <h1 className="text-4xl font-bold text-secondary-foreground  mb-8">
          Activity Feed
        </h1>
          ...
          {filteredActivities.map((activity, index) => {
            if (activity.type === "bet") {
              ...
              const timeAgo = bet.createdAt
                ...
                : "recently";
              const volume =
                parseFloat(bet.market.yesPool) + parseFloat(bet.market.noPool);
                        ...
                        <div className="flex items-start justify-between gap-4 mb-2">
                          <div>
                            ...
                            <p className="text-muted-foreground text-sm mt-1">
                              ...
                              {parseFloat(bet.amount).toFixed(2)} SOL
                              {bet.probability &&
                  ...
                  <Card className="p-6 bg-secondary hover:bg-background/70 hover-elevate active-elevate-2 transition-all duration-200 border border-border cursor-pointer cursor-pointer">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground">
                          <span>Final Probability: {market.probability}%</span>
                          <Badge variant="secondary" className="text-xs">
                            {market.category}
                          </Badge>
                        </div>
              ...
              return (
                ...
                >
                  <Card className="p-6 bg-secondary hover:bg-background/50 hover-elevate active-elevate-2 transition-all duration-200 cursor-pointer border border-border cursor-pointer">
                    <div className="flex items-start gap-4">
                      <div className="hidden md:block p-3 rounded-full text-secondary-foreground">
                        <Target className="h-5 w-5" />
                      </div>
                      <div className="flex-1">
                        <div className="flex items-start justify-between gap-4 mb-2">
